    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for m in reversed(data.get("messages", [])):
                direction = "→"
                inbound = m.get("recipient", "").lower() == receiver_address.lower()
                prefix = "IN " if inbound else "OUT"
                # Decrypt only inbound messages (we have the recipient's private key)
                plaintext = None
                if inbound:
                    plaintext = await fetch_and_decrypt(m["cid"], None)
                tail = plaintext if plaintext is not None else m["cid"]
                print(f"[{m['timestamp']}] {prefix} {m['sender']} {direction} {m['recipient']} (sid {m['sessionId'][:8]}…): {tail}")
        else:
